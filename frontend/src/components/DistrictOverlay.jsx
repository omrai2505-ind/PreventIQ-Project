...
const TABS = [
  ...
  { id: "ml",      label: "⬡  ML Analysis",     short: "ML"        },
];
...
const DistrictOverlay = ({ district, detail, loading, onClose }) => {
  ...
  return (
    <>
      ...
      {/* ── Main Panel ── */}
      <div style={S.panel} className="overlay-enter" onClick={(e) => e.stopPropagation()}>

        {/* Corner glows */}
        <div style={S.glowTL} />
        <div style={S.glowBR} />
        {mlData?.ml_backed && <div style={S.glowML} />}

        {/* Close */}
        <button style={S.closeBtn} onClick={onClose} title="ESC">✕</button>
          ...
          {TABS.map((t) => (
            ...
            >
              ...
              {t.id === "ml" && mlData?.ml_backed && (
                <span style={{
                  marginLeft: 6, fontSize: 9, background: "#00ff9d15",
                  border: "1px solid #00ff9d44", borderRadius: 4,
                  padding: "1px 5px", color: "#00ff9d",
                }}>LIVE</span>
              )}
            </button>
          ))}
          {/* ML accuracy badge — top right */}
          {mlData?.model_accuracy && (
            <div style={S.accBadge}>
              <div style={{ width: 5, height: 5, borderRadius: "50%", background: "#00ff9d",
                boxShadow: "0 0 6px #00ff9d", animation: "dotPulse 2s infinite" }} />
              <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: "#00ff9d" }}>
        ...
        <div style={S.body}>
          ...
          {activeTab === "ai" && (
            <RightPanel disease={disease} diseaseData={diseaseData}
              risk={district?.risk || "medium"} mlRisk={mlData?.risk_level} />
          )}
          {activeTab === "ml" && (
...
const S = {
  ...
  glowML: {
    position: "absolute", top: "40%", right: 0, width: 120, height: 200,
